...
import vn.edu.hust.formtest.GraduationProject
...
class UserAccountViewModel @Inject constructor(
    ...
    fun updateUser(user: User, imageUri: Uri?){
        ...
        if (!areInputsValid){
            viewModelScope.launch {
                _user.emit(Resource.Error("Check the inputs"))
            }
            return
        }
        ...
        viewModelScope.launch{
            try {
                val imageBitmap = MediaStore.Images.Media.getBitmap(
                    getApplication<GraduationProject>().contentResolver,
                    imageUri
                )
                val byteArrayOutputStream = ByteArrayOutputStream()
                imageBitmap.compress(Bitmap.CompressFormat.JPEG, 98, byteArrayOutputStream)
